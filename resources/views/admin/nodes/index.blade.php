...
@section('content-header')
    <h1>Nodes<small>Tất cả các node có sẵn trên hệ thống.</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Danh Sách Node</h3>
                <div class="box-tools search01">
                    <form action="{{ route('admin.nodes') }}" method="GET">
                        <div class="input-group input-group-sm">
                            <input type="text" name="filter[name]" class="form-control pull-right" value="{{ request()->input('filter.name') }}" placeholder="Tìm Kiếm Nodes">
                            <div class="input-group-btn">
                                <button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
                                <a href="{{ route('admin.nodes.new') }}"><button type="button" class="btn btn-sm btn-primary" style="border-radius: 0 3px 3px 0;margin-left:-1px;">Tạo Mới</button></a>
                            </div>
                        </div>
                    </form>
                </div>
    ...
    (function pingNodes() {
        $('td[data-action="ping"]').each(function(i, element) {
            $.ajax({
                ...
                var errorText = 'Lỗi kết nối đến node! Kiểm tra console trình duyệt để biết chi tiết.';
                try {
