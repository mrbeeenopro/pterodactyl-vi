...
export default ({ meta }: { meta: Record<string, unknown> }) => {
    ...
    return (
        <div className={'self-center md:px-4'}>
            <Dialog open={open} onClose={() => setOpen(false)} hideCloseIcon title={'Dữ liệu Metadata'}>
                <pre
                    className={
                ...
                <Dialog.Footer>
                    <Button.Text onClick={() => setOpen(false)}>Đóng</Button.Text>
                </Dialog.Footer>
            </Dialog>
            <button
                aria-describedby={'Xem thêm thông tin sự kiện'}
                className={
