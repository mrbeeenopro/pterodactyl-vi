    ...
    const submit = ({ name }: FormikValues, { setSubmitting }: FormikHelpers<FormikValues>) => {
        ...
        if (files.length === 1) {
            if (!useMoveTerminology && len === 1) {
                // Đổi tên tệp trong thư mục này.
                mutate((data) => data.map((f) => (f.name === files[0] ? { ...f, name } : f)), false);
            } else if (useMoveTerminology || len > 1) {
                // Xóa tệp khỏi thư mục này vì đã di chuyển nó đến nơi khác.
                        ...
                        <div css={[tw`flex flex-wrap`, useMoveTerminology ? tw`items-center` : tw`items-end`]}>
                            <div css={tw`w-full sm:flex-1 sm:mr-4`}>
                                <Field
                                    ...
                                    description={
                                        useMoveTerminology
                                            ? 'Nhập tên và thư mục mới của tệp hoặc thư mục này, tương đối với thư mục hiện tại.'
                                            : undefined
                                    }
                                    autoFocus
                                />
                            ...
                            <div css={tw`w-full sm:w-auto mt-4 sm:mt-0`}>
                                <Button css={tw`w-full`}>{useMoveTerminology ? 'Di chuyển' : 'Đổi tên'}</Button>
                            </div>
                        </div>
                        {useMoveTerminology && (
                            <p css={tw`text-xs mt-2 text-neutral-400`}>
                                <strong css={tw`text-neutral-200`}>Vị trí mới:</strong>
