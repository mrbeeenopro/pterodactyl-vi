...
const getActionDetails = (action: string): [string, any] => {
    switch (action) {
        case 'command':
            return ['Gửi Lệnh', faCode];
        case 'power':
            return ['Gửi Hành Động Nguồn', faToggleOn];
        case 'backup':
            return ['Tạo Bản Sao Lưu', faFileArchive];
        default:
            return ['Hành Động Không Xác Định', faCode];
    }
};
    ...
    return (
        <div css={tw`sm:flex items-center p-3 sm:p-6 border-b border-neutral-800`}>
            ...
            <ConfirmationModal
                ...
                buttonText={'Xóa Tác Vụ'}
                onConfirmed={onConfirmDeletion}
                visible={visible}
                onModalDismissed={() => setVisible(false)}
            >
                Bạn có chắc chắn muốn xóa tác vụ này không? Hành động này không thể hoàn tác.
            </ConfirmationModal>
            ...
            <div css={tw`flex-none sm:flex-1 w-full sm:w-auto overflow-x-auto`}>
                ...
                {task.payload && (
                    <div css={tw`md:ml-6 mt-2`}>
                        {task.action === 'backup' && (
                            <p css={tw`text-xs uppercase text-neutral-400 mb-1`}>Bỏ qua các tệp & thư mục:</p>
                        )}
                        <div
            ...
            <div css={tw`mt-3 sm:mt-0 flex items-center w-full sm:w-auto`}>
                ...
                <Can action={'schedule.update'}>
                    <button
                        type={'button'}
                        aria-label={'Chỉnh sửa tác vụ đã lên lịch'}
                ...
                <Can action={'schedule.update'}>
                    <button
                        type={'button'}
                        aria-label={'Xóa tác vụ đã lên lịch'}
