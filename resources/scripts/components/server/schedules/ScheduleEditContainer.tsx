...
export default () => {
    ...
    return (
        <PageContentBlock title={'Lịch trình'}>
                ...
                <>
                    ...
                    <div css={tw`rounded shadow`}>
                        ...
                        >
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    Lần chạy cuối:&nbsp;
                                    {schedule.lastRunAt ? (
                                        format(schedule.lastRunAt, "MMM do 'lúc' h:mma")
                                    ) : (
                                        <span css={tw`text-neutral-300`}>không có</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        Lần chạy tiếp theo:&nbsp;
                                        {schedule.nextRunAt ? (
                                            format(schedule.nextRunAt, "MMM do 'lúc' h:mma")
                                        ) : (
                                            <span css={tw`text-neutral-300`}>không có</span>
                                        )}
                                    </span>
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            <CronBox title={'Phút'} value={schedule.cron.minute} />
                            <CronBox title={'Giờ'} value={schedule.cron.hour} />
                            <CronBox title={'Ngày (Tháng)'} value={schedule.cron.dayOfMonth} />
                            <CronBox title={'Tháng'} value={schedule.cron.month} />
                            <CronBox title={'Ngày (Tuần)'} value={schedule.cron.dayOfWeek} />
                        </div>
